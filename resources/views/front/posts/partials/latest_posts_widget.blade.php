...
<div class="widget latest-posts">
    ...
    <div class="blog-posts">
        @foreach(App\Models\Post::query()->orderBy('created_at')->limit(3)->get() as $latestPost)
            ...
            <div class="item d-flex align-items-center">
                ...
                <div class="title"><strong>{{$latestPost->title}}</strong>
                    <div class="d-flex align-items-center">
                        @if((last(\Request::segments()) == $latestPost->id))
                        <div class="views"><i class="icon-eye"></i> {{$latestPost->visits_number + 1}}</div>
                        <div class="comments" id="comments-number-latest-posts"><i class="icon-comment"></i>{{$latestPost->comments_number}}</div>
                        @else
